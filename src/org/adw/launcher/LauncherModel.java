...
public class LauncherModel {
    ...
    public  ArrayList<ApplicationInfo> mApplicationsCust=new ArrayList<ApplicationInfo>();
    ...
    public ApplicationsAdapter getApplicationsAdapters(Launcher launcher)
    {
            ArrayList<ApplicationInfo>	mApplicationModle=new ArrayList<ApplicationInfo>();
            ApplicationsAdapter mApplicationsAdapterModle = new ApplicationsAdapter(launcher, mApplicationModle,
         		AppCatalogueFilters.getInstance().getDrawerFilter());
    	return mApplicationsAdapterModle;
    }
    ...
    static void removePackageByDrawer(Context context, String packageName) {
        final ContentResolver contentResolver = context.getContentResolver();
		final Cursor c = contentResolver.query(
				LauncherSettings.Favorites.CONTENT_URI, null,
				LauncherSettings.Favorites.SELECT_MODE + "= 3 and "
        		...
        		Intent intent=null;
                 ...
                 if(intent!=null) {
                	 ...
                	 if(mComponentName!=null) {
                		 if(packageName.equals(mComponentName.getPackageName())) {
                			 contentResolver.delete(LauncherSettings.Favorites.CONTENT_URI_NO_NOTIFICATION,
                						LauncherSettings.Favorites._ID + "=?", new String[] { String
                								.valueOf(id) });
                		 }
                	 }
                 }
    ...
    synchronized void removePackage(Launcher launcher, String packageName) { 
        ...
        if (packageName != null && packageName.length() > 0 && mApplicationsAdapter!=null) {
            ...
            final ContentResolver cr = launcher.getContentResolver();
...
//            						.valueOf(applicationInfo.id) });
    ...
    void loadUserItems(boolean isLaunching, Launcher launcher, boolean localeChanged,
            boolean loadApplications,int mode) {
       ...
       /* if (isLaunching && isDesktopLoaded()) {  几种模式切换
    ...
    private class DesktopItemsLoader implements Runnable {
        ...
        private void load_workspace() {  
            ...
            final HashMap<Long, FolderInfo> folders = new HashMap<Long, FolderInfo>();
             //LauncherSettings.Favorites.DISPLAY_MODE+ "= "+modes+""
            final Cursor c = contentResolver.query(
                    LauncherSettings.Favorites.CONTENT_URI, null,LauncherSettings.Favorites.SELECT_MODE+ "= "+modes+"" , null, null);
            try {
                final int idIndex = c.getColumnIndexOrThrow(LauncherSettings.Favorites._ID);
                final int intentIndex = c.getColumnIndexOrThrow(LauncherSettings.Favorites.INTENT);
                final int titleIndex = c.getColumnIndexOrThrow(LauncherSettings.Favorites.TITLE);
                ...
                while (!mStopped && c.moveToNext()) {
                    try {
                        ...
                        case LauncherSettings.Favorites.ITEM_TYPE_WIDGET_REMIND_CALENDAR:
                            widgetInfo = Widget.makeRemindCalendar();
    ...
    private static ApplicationInfo getApplicationInfo(PackageManager manager, Intent intent,
        ...
        System.out.println("info______________________>title"+info.title);
...
//                    info.icon =  Utilities.drawReflection(resources.getDrawable(id), context);
                } catch (Exception e) {
    ...
    static void addOrMoveItemInDatabase(Context context, ItemInfo item, long container,
            int screen, int cellX, int cellY,int mode) {
    	
        if (item.container == ItemInfo.NO_ID) {
            // From all apps
            addItemToDatabase(context, item, container, screen, cellX, cellY, false,mode);
        } else {
            // From somewhere else
            moveItemInDatabase(context, item, container, screen, cellX, cellY,mode);
        }
    }
    ...
    static void moveItemInDatabase(Context context, ItemInfo item, long container, int screen,
        ...
        values.put(LauncherSettings.Favorites.SELECT_MODE, Launcher.stateMod);//bob mode
        System.out.println(" 添加 ======================= " + Launcher.stateMod);
    ...
    static void moveItemInDatabase(Context context, ItemInfo item, long container, int screen,
        ...
        values.put(LauncherSettings.Favorites.CELLY, item.cellY);
        values.put(LauncherSettings.Favorites.SCREEN, item.screen);
        values.put(LauncherSettings.Favorites.SELECT_MODE, mode);//bob mode
        cr.update(LauncherSettings.Favorites.getContentUri(item.id, false), values, null, null);
    }
    ...
    static void addItemToDatabaseInstall(Context context, ItemInfo item, long container,
            int screen, int cellX, int cellY, boolean notify) {
        item.container = container;
        item.screen = screen;
        item.cellX = cellX;
        ...
        final ContentValues values = new ContentValues();
        final ContentResolver cr = context.getContentResolver();
        values.put(LauncherSettings.Favorites.SELECT_MODE,2);//bob mode
        item.onAddToDatabase(values);
    ...
    static void addItemToDatabase(Context context, ItemInfo item, long container,
            int screen, int cellX, int cellY, boolean notify,int mode) {
        item.container = container;
        item.screen = screen;
        item.cellX = cellX;
        item.cellY = cellY;
