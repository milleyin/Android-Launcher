...
//    private Drawable mNextIndicator;  bob
    //rogro82@xda
    int mHomeScreens = 0;
...
//        System.out.println("  mHomeScreens   is ---->  " +  (mHomeScreens));
    ...
    void setCurrentScreen(int currentScreen) {
        ...
        //ADW: dots
//        indicatorLevels(mCurrentScreen); bob
    ...
    private void updateWallpaperOffset(int scrollRange) {
    	...
    	if(getScrollX()>0 && getScrollX()<getChildAt(getChildCount() - 1).getLeft()){
    		mWallpaperManager.setWallpaperOffsetSteps(1.0f / (getChildCount() - 1), 1 );
    		mWallpaperManager.setWallpaperOffsets(getWindowToken(), getScrollX() / (float) scrollRange, 1);
    	}
    }
...
//        		canvas.drawBitmap(mWallpaperDrawable.getBitmap(), 0.0f, 0.0f, mPaint);
            	canvas.drawBitmap(mWallpaperDrawable.getBitmap(), x, y, mPaint);
			} catch (Exception e) {
//				e.printStackTrace();
//				canvas.drawBitmap(mWallpaperDrawable.getBitmap(), getScrollX(), mWallpaperY, mPaint);
			}
        }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
    	...
    	if(!lwpSupport){
    		if (mWallpaperLoaded && mWallpaperDrawable !=null) {
...
//                        System.out.println("TOUCH_STATE_SCROLLING>>>>>>>>>>>>>>>>>>>>"+TOUCH_STATE_SCROLLING);
                        enableChildrenCache();
...
//            	System.out.println("Workspace mTouchState>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>:"+mTouchState);
...
//		System.out.println("TOUCH_STATE_SCROLLING____________________________________"+TOUCH_STATE_SCROLLING);
		if (mTouchState == TOUCH_STATE_SCROLLING) {
...
//				System.out.println("else OnFling");
    ...
    void snapToScreen(int whichScreen) {
        ...
        //ADW: dots
//        indicatorLevels(mNextScreen); bob
...
//    	 System.out.println("onDropExternal");
...
//    	 System.out.println("onDropexternaldd");
...
//    	 System.out.println("onDropCompleted");
        clearVacantCache();
        if (success){
...
//        mNextIndicator = next;
	...
	public void setWallpaper(boolean fromIntentReceiver){
		...
		{
			mWallpaperDrawable.setCallback(null);
...
//    	indicatorLevels(mCurrentScreen); bob
    ...
    public CellLayout addScreen(int position){
    	...
    	screen.setOnLongClickListener(mLongClickListener); //bob longListener
    	if(mLauncher.getDesktopIndicator()!=null)mLauncher.getDesktopIndicator().setItems(getChildCount());
//    	indicatorLevels(mCurrentScreen); bob
    	AlmostNexusSettingsHelper.setDesktopScreens(mLauncher, getChildCount(),mLauncher.getMode());
