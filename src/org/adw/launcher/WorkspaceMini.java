...
import com.dongji.ui.MiniLinearLayout;
import com.dongji.ui.MiniLinearLayout.OnTouchActionUpListener;
 ...
 */
public class WorkspaceMini extends WidgetSpace implements DropTarget, DragSource, DragScroller,
												MultiTouchObjectCanvas<Object>, FlingListener,OnLongClickListener {
...
//    private int mWallpaperY;
     ...
     */
    public static boolean isInterceptTouchAble = true;

    LinearLayout selected_mini_screen;
    LinearLayout selected_mini_screen_bg;
    
    Bitmap mini_bitmap ; //mimiLauncher的状态
    Drawable wallPaperDrawable ; //壁纸
    ...
    ImageView animation_icon; //动画图片
    private static int ANIMATION_TIME = 400; //动画的时间
    
    View selected_view;
    
    int temp_screen;
    int [] tmep_spans = new int[2];
...
//    int screen;
    ...
    boolean isAddBySelected = false; //是否在选择屏幕的同时添加应用 或者 widget
    boolean isAddAble = true;
    Object  add_target ;   //需要被添加的目标  
    View target_view;  //目标的view
    
    Intent data;
    
    List<View> added_view = new ArrayList<View>();
    
    public WorkspaceMini(Context context, AttributeSet attrs) {
     ...
     */
    public WorkspaceMini(Context context, AttributeSet attrs, int defStyle) {
...
//        Launcher.setScreen(mCurrentScreen);
    ...
    public void init(  int size , int cur ,Bitmap mini_bitmap)
    {
    	if( animation_icon !=null)
    	{
...
//    	snapToScreen(0);
    	...
    	this.mini_bitmap = mini_bitmap;
    	wallPaperDrawable = WallpaperManager.getInstance(context).getDrawable();
    	
    	for(int i = 0; i<child_count ; i++)
    	{
    		LinearLayout ln = (LinearLayout) LayoutInflater.from(context).inflate(R.layout.mini_screen,null);
    		
    		if(i!=child_count-1)
    		{
    			ln.addView(createCellLayout(mini_bitmap, wallPaperDrawable));
    			ln.addView(createCellLayout(mini_bitmap, wallPaperDrawable));
    			ln.addView(createCellLayout(mini_bitmap, wallPaperDrawable));
    			...
    			if(yushu != 0)
    			{
    				for(int j = 0 ; j<yushu ;j++)
    				{
    					...
    					if(yushu ==1)
        				{
//        					ln.addView(createAddScreenCellLayout());
    						ln.addView(createEmptyCellLayout());
        					ln.addView(createEmptyCellLayout());
        				}else if(yushu ==2){
...
//        			if(size<9) //最多9个
//        			{
//        				LinearLayout ln_last = (LinearLayout) LayoutInflater.from(context).inflate(R.layout.mini_screen,null);
//            			ln_last.addView(createAddScreenCellLayout());
//            			
//            			ln_last.addView(createEmptyCellLayout());
//            			ln_last.addView(createEmptyCellLayout());
//            			
//            			addView(ln_last);
//        			}
...
//    			}
    	...
    	//选中当前屏幕
//    	 final LinearLayout ln = (LinearLayout) getChildAt(0);
//         FrameLayout fra =  (FrameLayout) ln.getChildAt(0);
//         LinearLayout ln1 = (LinearLayout)fra.getChildAt(0);
//         
//         ln1.setBackgroundColor(context.getResources().getColor(R.color.workspace_mini_item_selected_bg));
//         selected_mini_screen_bg = ln1;
//         LinearLayout ln2 = (LinearLayout)fra.getChildAt(1);
//         selected_mini_screen = ln2;
         
        final int child_index =  cur/3;
        int yu_shu = cur % 3;
         
        final LinearLayout ln = (LinearLayout) getChildAt(child_index);
         
        FrameLayout fra =  (FrameLayout) ln.getChildAt(yu_shu);
        ...
        LinearLayout lnn = (LinearLayout)fra.getChildAt(1);
        selected_mini_screen = lnn;
        
        post(new Runnable() {
			
			@Override
			public void run() {
				 setToScreen(child_index);
			}
		});
    }
...
//        }
    ...
    void addInScreen(View child, int screen, int x, int y, int spanX, int spanY, boolean insert) {
        ...
        cellLayout.addView(child, insert ? 0 : -1, lp);
...
//        System.out.println(" return   is  " + (mTouchState != TOUCH_STATE_REST));
...
//        indicatorLevels(mNextScreen);
        ...
        if(!mSensemode)
        	...
        	changeSelected(oldScreen , whichScreen);
    ...
    private void changeSelected(int oldScreen, int whichScreen) {
    	
//    	System.out.println("  whichScreen  --> " + whichScreen);
    	
    	if(oldScreen != whichScreen)
    	{
    		if(selected_mini_screen_bg !=null)
    		{
    			selected_mini_screen_bg.setBackgroundColor(Color.TRANSPARENT);
    		}
    		 final LinearLayout ln = (LinearLayout) getChildAt(whichScreen);
...
//        clearVacantCache();
//        if (success){
//            if (target != this && (mDragInfo != null || folderDragInfo!=null)) {
//            	
//            	 final Object tag ;
//            	 
//            	if(mDragInfo != null)
//            	{
//            		tag= mDragInfo.cell.getTag();
...
//                
//                
//                if(!(target instanceof FolderIcon) && target.getTag()!=null) //应用叠应用
//                {
//                	ApplicationInfo target_info = (ApplicationInfo)target.getTag();
//                	System.out.println("t.cellY  --- > " + target_info.cellX);
//                	System.out.println("t.cellY  --- >" + target_info.cellY);
//                	
//                	final CellLayout cellLayout = (CellLayout) getChildAt(getCurrentScreen());
//                	cellLayout.removeView(target); //移除被叠加的应用 
//                	
//                	//添加文件夹
...
//
//                     // Update the model
//                     LauncherModel.addItemToDatabase(mLauncher, folderInfo, LauncherSettings.Favorites.CONTAINER_DESKTOP,
//                             getCurrentScreen(), target_info.cellX, target_info.cellY, false);
//                     Launcher.getModel().addDesktopItem(folderInfo);
//                     Launcher.getModel().addFolder(folderInfo);
//
//                     // Create the view
//                     FolderIcon newFolder = FolderIcon.fromXml(R.layout.folder_icon, mLauncher,
//                             (ViewGroup)getChildAt(getCurrentScreen()), folderInfo);
////                     if(themeFont!=null)((TextView)newFolder).setTypeface(themeFont);
//                     addInCurrentScreen(newFolder,target_info.cellX, target_info.cellY, 1, 1, false);
//                     
//                     
//                     newFolder.mInfo.add(target_info);
//                     LauncherModel.addOrMoveItemInDatabase(mLauncher, target_info, newFolder.mInfo.id, 0, 0, 0);
//                     
//                     
//                     newFolder.mInfo.add((ApplicationInfo)tag);
//                     LauncherModel.addOrMoveItemInDatabase(mLauncher, (ApplicationInfo)tag, newFolder.mInfo.id, 0, 0, 0);
//                     
//                     newFolder.refresh();
//                }
//            }
...
//        if(mLauncher!=null)mWallpaperY=h - mLauncher.getWindow().getDecorView().getHeight();
    ...
    View createCellLayout(Bitmap mini_bitmap,Drawable wallPaperDrawable)
    {
    	LinearLayout.LayoutParams mini_lp = new LinearLayout.LayoutParams(LinearLayout.LayoutParams.FILL_PARENT, LinearLayout.LayoutParams.WRAP_CONTENT);
    	mini_lp.height = 20;
    	
    	
    	LinearLayout.LayoutParams far_lp  = new LinearLayout.LayoutParams(LinearLayout.LayoutParams.FILL_PARENT, LinearLayout.LayoutParams.FILL_PARENT, 1);
    	far_lp.leftMargin= 3;
    	far_lp.rightMargin= 2;
    	
    	FrameLayout fra = new FrameLayout(context);
    	fra.setLayoutParams(far_lp);
    	...
    	final LinearLayout bg_ln = new LinearLayout(context);
    	bg_ln.setLayoutParams(lp);
    	
    	fra.addView(bg_ln);
    	
    	LinearLayout.LayoutParams cell_lp  = new LinearLayout.LayoutParams(LinearLayout.LayoutParams.FILL_PARENT, LinearLayout.LayoutParams.FILL_PARENT, 1);
    	
    	FrameLayout.LayoutParams s_lp  = new FrameLayout.LayoutParams(FrameLayout.LayoutParams.FILL_PARENT, FrameLayout.LayoutParams.FILL_PARENT);
    	s_lp.topMargin= 3;
    	s_lp.leftMargin  = 3;
    	s_lp.rightMargin = 3;
    	s_lp.bottomMargin = 3;
    	
    	final MiniLinearLayout linear = new MiniLinearLayout(context);
		linear.setLayoutParams(s_lp);
		linear.setOrientation(LinearLayout.VERTICAL);
		linear.addView((CellLayout)LayoutInflater.from(context).inflate(R.layout.workspace_mini_screen, this, false), cell_lp);
		linear.setBackgroundDrawable(wallPaperDrawable);
		
		ImageView mini_img = new ImageView(context);
		mini_img.setImageBitmap(mini_bitmap);
    	mini_img.setLayoutParams(mini_lp);
    	
		linear.addView(mini_img);
		linear.setClickable(true);
		linear.setTag(temp);
		temp ++;
		
		linear.setOnTouchActionUpListener(new OnTouchActionUpListener() {
			...
			public void onUp() {
				if(selected_mini_screen_bg!=null)
					...
					selected_mini_screen = linear;
					selected_mini_screen_bg = bg_ln;
					if(!isAddBySelected)
					{
						selected_mini_screen_bg.setBackgroundColor(context.getResources().getColor(R.color.workspace_mini_item_selected_bg));
					}
					
					if(isAddBySelected && isAddAble)
					{
						addAppOrWidget();
					}
			}
...
//				selected_mini_screen = linear;
//				selected_mini_screen_bg = bg_ln;
//				selected_mini_screen_bg.setBackgroundResource(R.drawable.mini_sreen_selected_bg);
//			}
//		});
...
//		fra.addView(tv);
    ...
    View createAddScreenCellLayout()
    {
    	...
    	final FrameLayout fra = new FrameLayout(context);
    	fra.setLayoutParams(far_lp);
		...
		linear.setOrientation(LinearLayout.VERTICAL);
		linear.addView((CellLayout)LayoutInflater.from(context).inflate(R.layout.workspace_mini_screen, this, false), cell_lp);
		linear.setBackgroundDrawable(context.getResources().getDrawable(R.drawable.add_screen_bg));
		linear.setTag(temp);
		temp ++;
		...
		img.setLayoutParams(s_lp);
		img.setScaleType(ScaleType.CENTER_INSIDE);
		img.setEnabled(false);
		img.setClickable(false);
		img.setFocusable(false);
		img.setImageResource(R.drawable.add_screen_icon);
		...
		linear.setOnTouchActionUpListener(new OnTouchActionUpListener() {
			...
			public void onUp() {
				...
				launcher.mWorkspace.addScreen((Integer)linear.getTag());
				...
				linear.startAnimation(a);
				
				((ImageView)linear.getChildAt(1)).setImageBitmap(mini_bitmap);
				fra.removeView(img);
				
				int now_size = (Integer)linear.getTag() + 1;
				
				int parent = now_size / 3;
				int i = now_size  % 3; //余数
    ...
    void addAppIn(View view,ApplicationInfo appInfo)
    {
    	
    	
    	if(!isAddScreen() && isAddAble)
    	{
    		isAddAble  = false;
    		CellLayout cellLayout = (CellLayout)selected_mini_screen.getChildAt(0);
        	if(cellLayout.doSomething())
        	{

            	CellInfo cellInfo = cellLayout.getTag();
//            	System.out.println(" cellInfo  screen ---> " +cellInfo.screen );

            	if(cellInfo.findCellForSpan(mCellCoordinates, 1, 1, true))
            	{
            		
           		   final int screen = (Integer)selected_mini_screen.getTag();
           		   ...
           		   final ApplicationInfo info = new ApplicationInfo();
           		    ...
           		    {
           		    	info.icon =new BitmapDrawable(((FastBitmapDrawable)appInfo.icon).getBitmap());
           		    }else{
           		    	info.icon =new BitmapDrawable(((BitmapDrawable)appInfo.icon).getBitmap());
           		    }
                    ...
                    source_View =  view;
                    final View v =  launcher.completeAddShortcut2(info,screen,mCellCoordinates[0],mCellCoordinates[1]);
        		    ...
        		    if(dm.widthPixels>480 && dm.widthPixels<620)
        		    {
    		           ...
    		           CellLayout cl = (CellLayout)lnn.getChildAt(0);
    		    	   bitmap = Bitmap.createScaledBitmap(b, cl.mCellWidth/2, cl.mCellHeight/3 * 2, false);
        		    ...
        		    launcher.temp_bitmaps.add(bitmap);
        		    ...
        		    img.setLongClickable(true);
           		    img.setOnLongClickListener(this); //长按监听
        		    img.setTag(v);
        		    img.setImageBitmap(bitmap);
        		    
        		    //竖屏处理
        		    if(dm.widthPixels<dm.widthPixels)
        		    {
        		    	 img.setPadding(3, 0, 3, (int)(35*dm.density)/2);
        		    }else{
        		        img.setPadding(3, 0, 3, 3);
        		    }
        		 
        		    
           		    addInScreen(img, screen, mCellCoordinates[0], mCellCoordinates[1], 1, 1);
           		    img.setVisibility(View.GONE);
           		 
           		    anima_view = img;
           		    
           		    triggleAnimation(screen,0);
            	}else{
            		Toast.makeText(context, "屏幕空间不足，请选择其他屏幕", Toast.LENGTH_SHORT).show();
            		isAddAble  = true;
            	}
        	}else{
        		Toast.makeText(context, "屏幕空间不足，请选择其他屏幕", Toast.LENGTH_SHORT).show();
    ...
    void addWidgetIn(View view,AppWidgetProviderInfo appWidgetInfo)
    {
    	
    	if(!isAddScreen() && animation_icon.getVisibility() != View.VISIBLE)
    	{
    		CellLayout cellLayout = (CellLayout)selected_mini_screen.getChildAt(0);
    		
    		final int [] spans = new int[2];
    	     ...
    	     int spanX = (appWidgetInfo.minWidth  + smallerSize) / smallerSize;
    	     int spanY = (appWidgetInfo.minHeight + smallerSize) / smallerSize;
        	...
        	{
        		spans[0]  = 1;
        	}else if(spans[0] > cellLayout.getmColumns()) {
        		spans[0] = cellLayout.getmColumns();
        	}
        	
        	if(spans[1] == 0)
        	{
        		spans[1] = 1;
        	}else if(spans[1] > cellLayout.getmRows()) {
        		spans[1] = cellLayout.getmRows();
        	}
        	
        	
        	if(appWidgetInfo.label.equals("动机提醒日历"))
        	{
        		spans[0] = 4;
        		spans[1] = 4;
        	}
        	
//        	System.out.println(" spanX  ---> " + spans[0]);
//        	System.out.println(" spanY  ---> " + spans[1]);
...
//            	System.out.println(" cellInfo  screen ---> " +cellInfo.screen );
            	...
            	if(cellInfo.findCellForSpan(mCellCoordinates, spans[0], spans[1], false))
            	{
            		 final int screen = (Integer)selected_mini_screen.getTag();
            		...
            		if(appWidgetInfo.label.equals("动机提醒日历"))
                	{
            			...
            			View v  = launcher.addRemindCalendar2(screen,mCellCoordinates[0],mCellCoordinates[1]);
             		   
             		    anima_view = v;
             		    anima_view.setDrawingCacheEnabled(true);
                	    triggleAnimation(screen,1);
                		 
                	}else{
                		
                		if(!launcher.configureOrAddAppWidget(appWidgetInfo))
                		{
                			 View v  = launcher.addWidget(appWidgetInfo, screen, mCellCoordinates, spans);
                			 anima_view = v;
                			 anima_view.setDrawingCacheEnabled(true);
                			 triggleAnimation(screen,1);
                		}
    ...
    void addWidgetIn(Intent data)
    {
    	...
    	if(!isAddScreen() && animation_icon.getVisibility() != View.VISIBLE)
    	{
    		...
    		CellLayout cellLayout = (CellLayout)selected_mini_screen.getChildAt(0);
    		 ...
    		 {
    			if(customWidget.equals(Launcher.REMIND_CALENDAR_WIDGET))
    			{
    				spans[0] = 4;
    				spans[1] = 4;
    			}
...
//            		 source_View =  view;
            		...
            		if(customWidget!=null && customWidget.equals(Launcher.REMIND_CALENDAR_WIDGET))
                	{
...
//                	    triggleAnimation(screen,1);
							...
							&& (customWidget
									.equals(Launcher.SHORTCUT_MY_APP_WIDGET)
									|| customWidget
											.equals(Launcher.SHORTCUT_CLOUD_APP_WIDGET) || customWidget
									.equals(Launcher.SHORTCUT_TASK_MANAGER_WIDGET))) {
                		addCustomWidget2(customWidget, mCellCoordinates);
                	}else{
                		
                		if(!launcher.configureOrAddAppWidget2(data))
                		{
//                			 View v  = launcher.addWidget(appWidgetInfo, screen, mCellCoordinates, spans);
                			isAddAble=false;
                		     View v  =  launcher.realAddWidget3(appWidgetInfo,screen,mCellCoordinates,spans,appWidgetId);
                			 anima_view = v;
//                			 anima_view.setDrawingCacheEnabled(true);
                			 
//                			 postUpdateWidget();
                			 
//                			 System.out.println("add widget:"+mCellCoordinates[0]+", "+mCellCoordinates[1]);
                			 
                			 View loading_v = LayoutInflater.from(context).inflate(R.layout.loading_widget_item, null);
                			 
                			 addInScreen(loading_v, screen, mCellCoordinates[0], mCellCoordinates[1], ss[0], ss[1]);
                			 
                			 postDelayed(new PostUpdate(loading_v, v, screen), 2000);
                		}
                	}
    ...
    void setNextAddScreen(int parent, int i)
    {
    	LinearLayout l = (LinearLayout)getChildAt(parent);
		final FrameLayout fra = (FrameLayout)l.getChildAt(i);
		final MiniLinearLayout  ln = (MiniLinearLayout)fra.getChildAt(1);
		final LinearLayout bg_ln = (LinearLayout)fra.getChildAt(0);
		ln.setBackgroundResource(R.drawable.add_screen_bg);
		...
		ln.setOnTouchActionUpListener(new OnTouchActionUpListener() {
			
			@Override
			public void onUp() {
				
				launcher.mWorkspace.addScreen((Integer)ln.getTag());
				...
				ln.setBackgroundDrawable(wallPaperDrawable);
				Animation a = AnimationUtils.loadAnimation(context, R.anim.fade_in_fast);
				ln.startAnimation(a);
				
				fra.removeView(img);
				
				((ImageView)ln.getChildAt(1)).setImageBitmap(mini_bitmap);
				
				
				int now_size = (Integer)ln.getTag() + 1;
				
				int parent = now_size / 3;
				int i = now_size % 3; //余数
				
//				System.out.println( " 现在的总数  --->" + now_size  +" parent  is  --->" + parent + " 余数 --->" + i);
				if(i==0)
				{
					if(now_size < 9) //换页数
					{
...
//		});
    ...
    private boolean isAddScreen()
    {
    	...
    	if(selected_mini_screen ==null)
    	{
    		return true;
    	}
    	ImageView mini_launcher =(ImageView)selected_mini_screen.getChildAt(1);
    	if(mini_launcher.getDrawable() ==null)
    	{
    		b = true;
    		Toast.makeText(context, "请创建新的屏幕", Toast.LENGTH_SHORT).show();
    	}
//    	if(se)
    	return b;
    }
...
//		System.out.println(" 长按 ----->");
		...
		View mWorkspaceView = (View)selected_view.getTag();
		ItemInfo item = (ItemInfo) mWorkspaceView.getTag();
		
		if(item instanceof UserFolderInfo  || item instanceof LiveFolderInfo) //文件夹不操作
		{
			return false;
		}
		...
		{
			name = (String) ((ApplicationInfo)item).title;
		...
		}else if( item instanceof LauncherAppWidgetInfo)
		{
			...
			AppWidgetProviderInfo apInfo=mAppWidgetManager.getAppWidgetInfo(launcherAppWidgetInfo.appWidgetId);
			if(apInfo!=null)
			{
				name = apInfo.label;
			}
		...
		Dialog dialog = new AlertDialog.Builder(context).setMessage("将  '"+name+"' 从屏幕中移除?").setIcon(android.R.drawable.ic_dialog_info).setPositiveButton("确定", new DialogInterface.OnClickListener() {
			...
			public void onClick(DialogInterface dialog, int which) {
				...
				if (item instanceof LauncherAppWidgetInfo) { //插件
		                model.removeDesktopAppWidget((LauncherAppWidgetInfo) item);
		                
		                final LauncherAppWidgetInfo launcherAppWidgetInfo = (LauncherAppWidgetInfo) item;
		                final LauncherAppWidgetHost appWidgetHost = launcher.getAppWidgetHost();
		                launcher.getWorkspace().unbindWidgetScrollableId(launcherAppWidgetInfo.appWidgetId);
		                if (appWidgetHost != null) {
		                    appWidgetHost.deleteAppWidgetId(launcherAppWidgetInfo.appWidgetId);
		                }
		                
		            } else {
		                model.removeDesktopItem(item); //应用
		            }
				...
				CellLayout cellLayout = (CellLayout) launcher.mWorkspace.getChildAt(screen);
				
//				CellLayout cc = (CellLayout)launcher.mWorkspace.getChildAt();
//				System.out.println(" cc is null " + (cellLayout==null));
				cellLayout.removeView(mWorkspaceView);
...
//				sdfasdf
			    ...
			    FrameLayout fra =  (FrameLayout) ln.getChildAt(yu_shu);
//			    System.out.println(" yu_shu --->" + yu_shu);
			        
			    LinearLayout lnn = (LinearLayout)fra.getChildAt(1);
			        
			    CellLayout cell = (CellLayout)lnn.getChildAt(0);
			    cell.removeView(selected_view);
	...
	public void addWidgetAfterSetConfig(Intent data) {
        ...
        final View v  = launcher.realAddWidget2(appWidgetInfo, temp_screen, mCellCoordinates, tmep_spans,appWidgetId);
        
        anima_view = v;
		
        triggleAnimation(temp_screen, 1);
	}

   public void addWidgetAfterSetConfig2(Intent data) {
		isAddAble=false;
        ...
        final View v  = launcher.realAddWidget3(appWidgetInfo, temp_screen, mCellCoordinates, tmep_spans,appWidgetId);
        anima_view = v;
		anima_view.setDrawingCacheEnabled(true);
		 postDelayed(new Runnable() {
			...
			@Override
			public void run() {
				anima_view.buildDrawingCache();
				ImageView img = new ImageView(context);
        		img.setPadding(5, 5, 5, 5);
        		img.setScaleType(ScaleType.CENTER_INSIDE);
        		img.setBackgroundDrawable(new BitmapDrawable(anima_view.getDrawingCache()));
					
        		addInScreen(img, temp_screen, mCellCoordinates[0], mCellCoordinates[1], ss[0], ss[1]);
	...
	public void setAnimation_icon(ImageView animation_icon) {
	...
	Handler handler = new Handler()
	{
		...
		public void handleMessage(android.os.Message msg) {
			...
			source_View.setDrawingCacheEnabled(true);
			source_View.buildDrawingCache();
			Bitmap bm = Bitmap.createBitmap(source_View.getDrawingCache());
     		  
			animation_icon.setImageBitmap(bm);
   		    
   		    //触发动画
   		    animation_icon.setVisibility(View.VISIBLE);
 		    TranslateAnimation ta = new TranslateAnimation(a_x, a_tx, a_y, a_ty);
 		    ta.setDuration(ANIMATION_TIME);
 		
 		    ScaleAnimation scaleAnimation = new ScaleAnimation(1.0f, 0.35f, 1.0f, 0.35f);
 		   ...
 		   animationSet.setAnimationListener(new AnimationListener() {
 			...
 			public void onAnimationEnd(Animation animation) {
 				...
 				{
 					animation_icon.setVisibility(View.GONE);
 	 				anima_view.setVisibility(View.VISIBLE);
 				}else{
 					animation_icon.setVisibility(View.GONE);
 					anima_view.setVisibility(View.VISIBLE);
				    ...
				    ImageView img = new ImageView(context);
	        		img.setScaleType(ScaleType.CENTER_INSIDE);
	        		Bitmap b = anima_view.getDrawingCache();
	        		img.setBackgroundDrawable(new BitmapDrawable(b));
	        		img.setLongClickable(true);
	           		img.setOnLongClickListener(WorkspaceMini.this); //长按监听
	           		img.setTag(anima_view);
           ...
           animation_icon.startAnimation(animationSet);
		};
	};
	 ...
	 * 
	 * @param screen
	 * @param type  0 应用  ; 1 插件
	 */
	void triggleAnimation(int screen,final int type)
	{
		final int s = screen;
		new Thread(new Runnable() {
			...
			public void run() {
       		 ...
       		 source_View.getHitRect(r1);
       		 View g = ((View)source_View.getParent());
       		 g.getHitRect(r2);
       		    
       		 View scrollLayoutTouchable = ((View)g.getParent());
       		 scrollLayoutTouchable.getHitRect(r3);
       		    
       		  View l = ((View)scrollLayoutTouchable.getParent());
       		  l.getHitRect(r4);
       		    
       		    View l2 = ((View)l.getParent());
       		    l2.getHitRect(r5);
       		    
       		    
       		     View baseFrame = ((View)l2.getParent());
       		    baseFrame.getHitRect(baseR);
       		    
       		    int x = r5.left+r4.left+r3.left + r2.left+ r1.left+baseR.left ;
       		    int y = r5.top+ r4.top+ r3.top+ r2.top + r1.top +baseR.top  ;
       		    
       		    if(x>scrollLayoutTouchable.getWidth())
       		    {
       		    	int num = x/scrollLayoutTouchable.getWidth();
       		    	x-= scrollLayoutTouchable.getWidth()*num;
       		    }
                ...
                int yu_shu = s % 3;
       		    ...
       		    System.out.println(" child_index  --->"  + child_index);
       		    System.out.println(" yu_shu  --->"  + yu_shu); */
                ...
                int cellWidth = cell.getWidth()/cell.getmColumns();
                int cellHight = cell.getHeight() / cell.getmRows();
                
                int tx = r44.left + r33.left + r22.left+ r11.left + mCellCoordinates[0] * cellWidth + cellWidth/3 ;
       		    int ty = r44.top + r33.top+ r22.top + r11.top + mCellCoordinates[1] * cellHight + cellHight/3;
       		    
       		    tx -= child_index*ln.getWidth();
       		    
//       		    System.out.println(" 目标   ---->   tx: " + tx +  " ty: "+ ty);
...
//	//zy:mark old
//	private void addAppOrWidget()
//	{
//		if(add_target instanceof AppWidgetProviderInfo) //添加widge
//		{
//			AppWidgetProviderInfo appWidgetInfo = (AppWidgetProviderInfo)add_target;
//			
//			final Resources resources = context.getResources();
//   	        int actualWidth = resources.getDimensionPixelSize(R.dimen.cell_width);
//   	        int actualHeight = resources.getDimensionPixelSize(R.dimen.cell_height);
...
//
//   	     // Always round up to next largest cell
//   	        int min_width = (appWidgetInfo.minWidth  + smallerSize) / smallerSize;
//   	        int min_height = (appWidgetInfo.minHeight + smallerSize) / smallerSize;
//   	        
//    		final int [] spans = new int[2];
//        	spans[0] = min_width;
//        	spans[1] = min_height;
...
//                	{
//                		
//            			 final View v  = launcher.addRemindCalendar3(screen,mCellCoordinates[0],mCellCoordinates[1]);
//             		   
////             		    anima_view = v;
...
//		    	        		img.setPadding(5, 5, 5, 5);
//		    	        		img.setScaleType(ScaleType.CENTER_INSIDE);
//		    	        		img.setBackgroundDrawable(new BitmapDrawable(v.getDrawingCache()));
...
//                	}else{
//                		
//                		if(!launcher.configureOrAddAppWidget2(appWidgetInfo,this))
//                		{
//                			 final View v  = launcher.addWidget2(appWidgetInfo, screen, mCellCoordinates, spans);
//                			 
////                			 anima_view = v;
//                			 v .setDrawingCacheEnabled(true);
//                			 v.destroyDrawingCache(); 
...
//            		
//            	}else{
//            		Toast.makeText(context, "屏幕空间不足，请选择其他屏幕或添加新的屏幕", Toast.LENGTH_SHORT).show();
//            	}
//        	}else{
//        		Toast.makeText(context, "屏幕空间不足，请选择其他屏幕或添加新的屏幕", Toast.LENGTH_SHORT).show();
//        	}
...
//		}else{  //添加应用
//			
//			ApplicationInfo appInfo = (ApplicationInfo)add_target;
//
//    		CellLayout cellLayout = (CellLayout)selected_mini_screen.getChildAt(0);
//        	if(cellLayout.doSomething())
//        	{
...
//                    info.spanY = 1;
//           		    
//           		    launcher.batchAddShortcut(info,screen,mCellCoordinates[0],mCellCoordinates[1]);
//           		     
//           		    target_view.setDrawingCacheEnabled(true);
//           		    target_view.buildDrawingCache();
//           		    
//           		    Bitmap bitmap = Bitmap.createBitmap(target_view.getDrawingCache());
//           		    
//           		    final ImageView img = new ImageView(context);
//        		    img.setPadding(5, 5, 5, 5);
//        		    img.setScaleType(ScaleType.CENTER_INSIDE);
...
//	                	    triggleAnimation(screen,1);
								...
								&& (customWidget
										...
										|| customWidget
												.equals(Launcher.SHORTCUT_CLOUD_APP_WIDGET) || customWidget
										.equals(Launcher.SHORTCUT_TASK_MANAGER_WIDGET))) {
	                		addCustomWidget(customWidget, mCellCoordinates, screen);
	                	} else{
	                		     isAddAble = false;
//	                			 View v  = launcher.addWidget(appWidgetInfo, screen, mCellCoordinates, spans);
	                		     View v  =  launcher.realAddWidget3(appWidgetInfo,screen,mCellCoordinates,spans,appWidgetId);
            	...
            	if(cellInfo.findCellForSpan(mCellCoordinates, 1, 1, true))
            	{
            	   ...
            	   isAddAble = false;
                    ...
                    info.spanY = 1;
           		    ...
           		    View v = launcher.batchAddShortcut(info,screen,mCellCoordinates[0],mCellCoordinates[1]);
...
//        		    (new BitmapDrawable(bitmap));
        		    ...
        		    if(width<height)
        		    {
        		    	 img.setPadding(0, 0, 0, (int)(35*dm.density)/2);
        		    }
	...
	private void addCustomWidget(String customWidget, int[] xy, int screen) {
		isAddAble = false;

		View v = launcher.addCustomShortcutWidgetByMini(customWidget, xy,
				screen);
...
//				System.out.println("--------------bitmap :"+b+", temp_screen:"+temp_screen+", "+mCellCoordinates[0]+", "+mCellCoordinates[1]+", "+ss[0]+", "+ss[1]);
				...
				addInScreen(img, temp_screen, mCellCoordinates[0],
						mCellCoordinates[1], 1, 1);
				isAddAble = true;
			}
		}, 300);
	}
	
	private void addCustomWidget2(String customWidget, int[] xy) {
		isAddAble=false;
		...
		postDelayed(new Runnable() {
			...
			public void run() {
	        	...
	        	addInScreen(img, temp_screen, mCellCoordinates[0], mCellCoordinates[1], 1, 1);
	        	isAddAble=true;
			}
		}, 2000);
	}
	
	public void changToAddMode(Object target, View target_view)
	{
		this.add_target = target;
		this.target_view = target_view;
		this.isAddBySelected = true;
		this.isAddAble = true;
		added_view.clear();
	}
	
	public void changToAddMode(Intent data)
	{
		this.add_target = new AppWidgetProviderInfo();
		this.data = data;
		this.isAddBySelected = true;
		this.isAddAble = true;
		added_view.clear();
	...
	class PostUpdate implements Runnable{
		...
		public PostUpdate(View loading_v,View target_view,int screen){
			this.loading_v = loading_v;
			this.target_view = target_view;
			this.screen = screen;
		}
		...
		public void run() {
		        ...
		        CellLayout cellLayout = (CellLayout)lnn.getChildAt(0);
		        cellLayout.removeView(loading_v);
				...
				ImageView img = new ImageView(context);
	            img.setScaleType(ScaleType.CENTER_INSIDE);
	        	Bitmap b = target_view.getDrawingCache();
	        	img.setBackgroundDrawable(new BitmapDrawable(b));
	           	img.setTag(target_view);
						
	        	addInScreen(img, screen, mCellCoordinates[0], mCellCoordinates[1], ss[0], ss[1]);
	...
	//撤销已添加的应用 或者 组件
	public void cancleAdded()
	{
		for(View mWorkspaceView:added_view)
		{
			ItemInfo item = (ItemInfo) mWorkspaceView.getTag();
