 ...
 */
public class DrawerWorkspace extends WidgetSpace implements DropTarget, DragSource, DragScroller,
...
//    private Drawable mNextIndicator;
    ...
    private ApplicationInfo folderDragInfo;  //从文件里面拖出来的应用
    ...
    public DrawerWorkspace(Context context, AttributeSet attrs) {
     ...
     */
    public DrawerWorkspace(Context context, AttributeSet attrs, int defStyle) {
...
//        Launcher.setScreen(mCurrentScreen);
    ...
    public int getScreenChildCount(int screen) {
    ...
    void setCurrentScreen(int currentScreen) {
        ...
        if(mDesktopIndicator!=null){
        	mDesktopIndicator.fullIndicate(mCurrentScreen);
    ...
    void addInScreen(View child, int screen, int x, int y, int spanX, int spanY, boolean insert) {
        ...
        if(screen >= getChildCount()) {
			int count = (screen - getChildCount()) + 1;
			LayoutInflater layoutInflter=LayoutInflater.from(mLauncher);
			for(int i=0;i<count;i++) {
				CellLayout screenLayout=(CellLayout)layoutInflter.inflate(R.layout.workspace_screen, this, false);
				screenLayout.setColumnAndRow(mDesktopColumns, mDesktopRows);
    ...
    public void computeScroll() {
        if (mScroller.computeScrollOffset()) {
            ...
            if(mDesktopIndicator!=null)mDesktopIndicator.indicate((float)mScroller.getCurrX()/(float)(getChildCount()*getWidth()));
            postInvalidate();
            ...
            clearChildrenCache();
            if(mDesktopIndicator!=null)mDesktopIndicator.fullIndicate(mCurrentScreen);
...
//        System.out.println("onTouchEvent:"+ev.getAction()+", mTouchState:"+mTouchState);
        ...
        case MotionEvent.ACTION_MOVE:
            ...
            if (mTouchState == TOUCH_STATE_SCROLLING) {
                ...
                scrollBy(deltaX, 0);
//                updateWallpaperOffset();
                if(mDesktopIndicator!=null)mDesktopIndicator.indicate((float)getScrollX()/(float)(getChildCount()*getWidth()));
            }
            break;
        case MotionEvent.ACTION_UP:
...
//        indicatorLevels(mNextScreen);
     ...
     * 设置抽屉指示图标
     */
    void setDesktopIndicator(DesktopIndicator mDesktopIndicator) {
    	this.mDesktopIndicator=mDesktopIndicator;
    }
     ...
     * @param allList
     */
    void hideApp(List<ApplicationInfo> allList) {
    	...
    	for(int i=0;i<getChildCount();i++) {
    		...
    		for(int j=0;j<cellLayout.getChildCount();j++) {
    			...
    			if(v.getTag() instanceof ApplicationInfo) {
    				ApplicationInfo aInfo=(ApplicationInfo)v.getTag();
    				int k=0;
	    			for(;k<allList.size();k++) {
	    				ApplicationInfo hideInfo=allList.get(k);
	    				if(aInfo.getPackageName().equals(hideInfo.getPackageName()) && aInfo.title.equals(hideInfo.title)) {
	    					if(aInfo.mSortEntity.visible!=hideInfo.mSortEntity.visible && hideInfo.mSortEntity.visible==1) {
	    						db.setVisible(aInfo.getPackageName(), 1, aInfo.title.toString());
    			...
    			}else if(v.getTag() instanceof UserFolderInfo) {
    				UserFolderInfo userInfo=(UserFolderInfo)v.getTag();
    				for(int x=0;x<userInfo.contents.size();x++) {
    					ApplicationInfo aInfo=userInfo.contents.get(x);
        				int k=0;
    	    			for(;k<allList.size();k++) {
    	    				ApplicationInfo hideInfo=allList.get(k);
    	    				if(aInfo.getPackageName().equals(hideInfo.getPackageName())) {
    	    					if(aInfo.mSortEntity.visible!=hideInfo.mSortEntity.visible && hideInfo.mSortEntity.visible==1) {
    	    						db.setVisible(aInfo.getPackageName(), 1, aInfo.title.toString());
    	    						userInfo.contents.remove(x--);
//    	    						cellLayout.removeView(v);
    	    						j--;
    	    						break;
    	    					}
    	...
    	final int maxColumn=mDesktopColumns-1;
    	final int maxRow=mDesktopRows;
        List<View> saveList=new ArrayList<View>();
        for(int i=0;i<getChildCount();i++) {
        	CellLayout cellLayout = (CellLayout) getChildAt(i);
    	...
    	for(int i=0;i<allList.size();i++) {
    		ApplicationInfo hideInfo=allList.get(i);
    		if(hideInfo.mSortEntity.visible==0) {
    			View v=mLauncher.createShortcut(hideInfo);
    			db.setVisible(hideInfo.getPackageName(), 0, hideInfo.title.toString());
    			saveList.add(v);
//    			bindItems.add(hideInfo);
    		}
    	}
        ...
        for(int i=0;i<saveList.size();i++) {
        	View v=saveList.get(i);
        	ItemInfo info=(ItemInfo)v.getTag();
        	info.screen=num[0];
        	info.cellX=num[1];
        	info.cellY=num[2];
        	CellLayout.LayoutParams mParams=(CellLayout.LayoutParams)v.getLayoutParams();
        	mParams.cellX=info.cellX;
        	mParams.cellY=info.cellY;
//        	System.out.println(info+", "+info.screen+", "+info.cellX+", "+info.cellY);
        	addInScreen(v, info.screen, info.cellX, info.cellY, 1, 1, true);
        	if(num[1]<maxColumn) {
				num[1]+=1;
			}else {
				num[1]=0;
        ...
        mDesktopIndicator.setItems(getChildCount());
        if(removed) {
        	setCurrentScreen(0);
        }
    }
    
    List<ItemInfo> getAllImtem() {
    	...
    	for(int i=0;i<getChildCount();i++) {
			...
			for(int j=0;j<count;j++) {
				View v=cellLayout.getChildAt(j);
				itemList.add((ItemInfo)v.getTag());
			}
    	}
    	return itemList;
    }
     ...
     * 当应用安装或卸载
     */
    List<ItemInfo> onPackageChanged(String packageName, boolean added) {
    	if(added) {
			PackageManager packageManager = mLauncher.getPackageManager();
			Intent allApplicationMainIntent = new Intent(Intent.ACTION_MAIN,
					null);
			...
			for (ResolveInfo info : all_apps) { 
				...
				if (activityInfo.packageName.equals(packageName)) {
					...
					final ApplicationInfo applicationinfo=getApplicationInfo(packageManager, intent, mLauncher);
					applicationinfo.title=title;
					
					int[] num=getEndPoint();
					applicationinfo.screen=num[0];
					applicationinfo.cellX=num[1];
					applicationinfo.cellY=num[2];
					
//					mLauncher.runOnUiThread(new Runnable() {
//						@Override
//						public void run() {
							// TODO Auto-generated method stub
							View v=mLauncher.createShortcut(applicationinfo);
							
							bindItems.add((ItemInfo)v.getTag());
							
							addInScreen(v, applicationinfo.screen, applicationinfo.cellX, applicationinfo.cellY, 1, 1,
				                    false);
...
//					});
    		...
    		/*for(int i=0;i<getChildCount();i++) {
    			...
    			for(int j=0;j<cellLayout.getChildCount();j++) {
    				...
    				if(info instanceof UserFolderInfo) {
    					UserFolderInfo userInfo=(UserFolderInfo)info;
    					for(int k=0;k<userInfo.contents.size();k++) {
    						ApplicationInfo aInfo=userInfo.contents.get(k);
    						if(aInfo.getPackageName().equals(packageName)) {
    							userInfo.contents.remove(k);
    							break;
    						}
    					}
    				}else if(info instanceof ApplicationInfo) {
    					...
    					if(aInfo.getPackageName().equals(packageName)) {
							...
							if (cellLayout.getChildCount() == 0) {
								...
								int[] num = new int[3];
								final int maxColumn = mDesktopColumns - 1;
								final int maxRow = mDesktopRows - 1;
								if (aInfo.cellX == maxColumn) {
									num[1] = 0;
									if (aInfo.cellY == maxRow) {
										num[0] = aInfo.screen + 1;
										num[2] = 0;
									} else {
										num[0] = aInfo.screen;
										num[2] = aInfo.cellY + 1;
    		...
    		removeItemInfoByPackageName(packageName);*/
    		...
    		for(int i=0;i<views.size();i++) {
    			CellLayout cellLayout = (CellLayout) getChildAt(num[0]);
            	View v=views.get(i);
            	ItemInfo info=(ItemInfo)v.getTag();
            	info.screen=num[0];
            	info.cellX=num[1];
    		...
    		List<ItemInfo> items=removeItemInfoByPackageName(packageName);
    ...
    private List<ItemInfo> removeItemInfoByPackageName(String packageName) {
    	List<ItemInfo> items=new ArrayList<ItemInfo>();
    	for(int i=0;i<bindItems.size();i++) {
    		ItemInfo info=bindItems.get(i);
    		if(info instanceof ApplicationInfo) {
    			ApplicationInfo aInfo=(ApplicationInfo)info;
    			if(aInfo.getPackageName().equals(packageName)) {
    				items.add(aInfo);
	...
	private ApplicationInfo getApplicationInfo(PackageManager manager,
		...
		DrawerDatabase db=new DrawerDatabase(mLauncher);
		info.mSortEntity=db.getSortEntityByPackageName(info.getPackageName(), info.title.toString());
    ...
    int[] getEndPoint() {
		...
		if (getChildCount() > 0) {
			...
			int count=cellLayout.getChildCount();
			num[0]=getChildCount() - 1;
    		for(int i=0;i<count;i++) {
    			View v=cellLayout.getChildAt(i);
    			ItemInfo info=(ItemInfo)v.getTag();
				if (info.screen>num[0] || (info.cellX >= num[1] && info.cellY == num[2]) || info.cellY>num[2]) {
					if(info.cellX<maxColumn) {
						num[0]=info.screen;
	    				num[1]=info.cellX+1;
	    				num[2]=info.cellY;
	    			}else {
    ...
    void setRecentInstallViews() {
    	removeAllViews();
    	DrawerDatabase db=new DrawerDatabase(mLauncher);
    	List<String[]> packageList=db.getPackageList();
    	if(packageList!=null && packageList.size()>0) {
    		List<ItemInfo> appList=new ArrayList<ItemInfo>();
    		for(int i=0;i<packageList.size();i++) {
    			String[] packageName=packageList.get(i);
    			for(int j=0;j<bindItems.size();j++) {
    				ApplicationInfo info=(ApplicationInfo)bindItems.get(j);
//    				System.out.println(info+"    "+info.getIntentString());
    				if(info.getPackageName().equals(packageName[0]) && info.title.toString().equals(packageName[1])) {
    		...
    		for(int i=0;i<appList.size();i++) {
				final ItemInfo item = appList.get(i);
				item.screen = num[0];
				item.cellX = num[1];
				item.cellY = num[2];
				final View shortcut = mLauncher
						.createShortcut((ApplicationInfo) item);
				addInScreen(shortcut, item.screen, item.cellX, item.cellY, 1,
						1, true);
				if(num[1]<maxColumn) {
    				num[1]+=1; 
    			}else {
     ...
     * 添加最近安装 View 
     */
    void addRecentInstallViews(String packageName, ItemInfo info) {
    	this.bindItems.add(info);
    	setRecentInstallViews();
    }
    
    void removeRecentInstallApplicationInfo(String packageName) {
    	for(int j=0;j<bindItems.size();j++) {
			ApplicationInfo info=(ApplicationInfo)bindItems.get(j);
    ...
    void removeRecentOpenApplicationInfo(String packageName) {
    	for(int j=0;j<bindItems.size();j++) {
			ApplicationInfo info=(ApplicationInfo)bindItems.get(j);
			if(info.getPackageName().equals(packageName)) {
				bindItems.remove(j);
     ...
     * 添加最近打开 View 
     */
    void addRecentOpenViews() {
    	removeAllViews();
    	DrawerDatabase db=new DrawerDatabase(mLauncher);
    	List<SortEntity> openList=db.getSortPackageList();
    	if(openList!=null && openList.size()>0) {
    		...
    		for(int i=0;i<openList.size();i++) {
    			...
    			for(int j=0;j<bindItems.size();j++) {
    				ApplicationInfo info=(ApplicationInfo)bindItems.get(j);
    				if(info.getPackageName().equals(entity.packageName)) {
    					appList.add(info);
    					break;
    				}
    			}
    ...
    void setDataList(List<ItemInfo> infos) {
    	bindItems.clear();
    	for(ItemInfo info : infos) {
    		if(info instanceof ApplicationInfo){
	    		ItemInfo i;
				try {
					i = (ItemInfo)info.clone();
					this.bindItems.add(i);
    ...
    void bindItemsByLetterSort(List<ItemInfo> shortcuts) {
    	Collections.sort(shortcuts, mComparator);
    	this.bindItems=shortcuts;
    }
    
    private List<ItemInfo> bindItems=new ArrayList<ItemInfo>();
    void sortByLetter() {
		Collections.sort(bindItems, mComparator);
		int[] num = new int[3];
		for(int i=0;i<bindItems.size();i++) {
			moveItem(num, bindItems.get(i));
		} 
		requestLayout();
		invalidate();
    }
...
//			System.out.println(object1+", "+object2+", "+str1+", "+str2);
    ...
    private void moveItem(int[] num, ItemInfo itemInfo) {
    	boolean flag=false;
    	final int maxColumn=mDesktopColumns-1;
    	final int maxRow=mDesktopRows;
    	ApplicationInfo aInfo=(ApplicationInfo)itemInfo;
    	for(int i=0;i<getChildCount();i++) {
    		CellLayout cellLayout = (CellLayout) getChildAt(i);
    		int j=0;
    		for(;j<cellLayout.getChildCount();j++) {
    			View v=cellLayout.getChildAt(j);
...
//    				System.out.println("++++"+itemInfo+", "+info.screen+", "+info.cellX+", "+info.cellY+", "+num[0]+", "+num[1]+", "+num[2]);
    				CellLayout.LayoutParams mParams=(CellLayout.LayoutParams)v.getLayoutParams();
    				info.cellX=num[1];
    				info.cellY=num[2];
    				mParams.cellX=info.cellX;
    				mParams.cellY=info.cellY;
//    				System.out.println(info+", "+info.screen);
    				if(info.screen!=num[0]) {
						info.screen = num[0];
						cellLayout.removeView(v);
						
						((CellLayout) getChildAt(info.screen)).addView(v, mParams);
    				}
    				flag=true;
    				break;
    			}
    ...
    private void initSortData(int type) {
    	...
    	if(sortList!=null && sortList.size()<=bindItems.size()) {
    		int[] dragArr=new int[3];
    		...
    		for(int i=0;i<sortList.size();i++) {
    			...
    			for(;j<getChildCount();j++) {
    				CellLayout cellLayout = (CellLayout) getChildAt(j);
    		    	int count=cellLayout.getChildCount();
    		    	for(int k=0;k<count;k++) {
    		    		View v=cellLayout.getChildAt(k);
    		    		ItemInfo info=(ItemInfo)v.getTag();
    		    		if((info instanceof UserFolderInfo && info.id==entity.id) || (info instanceof ApplicationInfo && ((ApplicationInfo)info).getPackageName().equals(entity.packageName))) {
    		    			isFind=true;
    		    			
    		    			CellLayout.LayoutParams mParams=(CellLayout.LayoutParams)v.getLayoutParams();
    		    			
    		    			dragArr[0]=info.screen;
    		    			dragArr[1]=mParams.cellX;
    		    			dragArr[2]=mParams.cellY;
    		    			
							if (tempArr[1] < maxColumn) { 
								if(!isFirst) {
									tempArr[1] += 1;
								}else {
									isFirst=false;
								...
								tempArr[1] = 0;
								tempArr[2] += 1;
								if (tempArr[2] % maxRow == 0) {
									tempArr[0] ++;
									tempArr[2] = 0;
								}
							}
    		    			if(dragArr[0]!=tempArr[0] || dragArr[1]!=tempArr[1] || dragArr[2]!=tempArr[2]) 
    		    				moveItemByPoint(dragArr, cellLayout, v, tempArr);
    		    			break;
    		    		}
    		    	}
    		    	if(isFind) {
    ...
    private void moveItemByPoint(int[] dragArr, CellLayout cellLayout, View dragView, int[] tempArr) {
    	for(int i=tempArr[0];i<getChildCount();i++) {
    		CellLayout cellLayout2 = (CellLayout) getChildAt(i);
	    	int count=cellLayout2.getChildCount();
	    	int x=0;
	    	for(;x<count;x++) {
	    		View v=cellLayout2.getChildAt(x);
	    		ItemInfo info=(ItemInfo)v.getTag();
	    		if(tempArr[0]==info.screen && tempArr[1]==info.cellX && tempArr[2]==info.cellY) {
		    		CellLayout.LayoutParams mParams=(CellLayout.LayoutParams)v.getLayoutParams();
		    		mParams.cellX=dragArr[1];
		    		mParams.cellY=dragArr[2];
		    		info.screen=dragArr[0];
		    		info.cellX=dragArr[1];
		    		info.cellY=dragArr[2];
		    		
//		    		v.setLayoutParams(mParams);
//		    		v.setTag(info);
		    		
		    		CellLayout.LayoutParams mDragParams=(CellLayout.LayoutParams)dragView.getLayoutParams();
		    		ItemInfo dragInfo=(ItemInfo)dragView.getTag();
		    		mDragParams.cellX=tempArr[1];
		    		mDragParams.cellY=tempArr[2];
		    		dragInfo.screen=tempArr[0];  
		    		dragInfo.cellX=tempArr[1];
		    		dragInfo.cellY=tempArr[2];
		    		 
//		    		dragView.setLayoutParams(mDragParams);
//		    		dragView.setTag(dragInfo);
		    		
		    		if(cellLayout!=cellLayout2) {
		    			cellLayout2.removeView(v);
		    			cellLayout.removeView(dragView);
		    			cellLayout2.addView(dragView);
			    		cellLayout.addView(v, mParams);   
		    		}
		    		break;
	    		}
	    	}
    ...
    void addNewFolder(String title, List<ApplicationInfo> container) {
    	UserFolderInfo folderInfo = new UserFolderInfo();
    	folderInfo.title=title;
    	folderInfo.itemType=LauncherSettings.Favorites.ITEM_TYPE_USER_FOLDER;
    	...
    	for(int i=0;i<getChildCount();i++) {
	    	...
	    	for(int j=0;j<count;j++) {
	    		...
	    		for(int k=0;k<container.size();k++) {
	    			if(container.get(k).id==info.id) {
	    				if(isFirst) {
	    					folderInfo.screen=info.screen;
	    					folderInfo.cellX=info.cellX;
	    					folderInfo.cellY=info.cellY;
	    					isFirst=false;
	    					LauncherModel.addItemToDatabase(mLauncher, folderInfo, LauncherSettings.Favorites.CONTAINER_DESKTOP,
	    				        		folderInfo.screen, folderInfo.cellX, folderInfo.cellY, false);
	    				}
	    				j--;
	    				count--;
	    				cellLayout.removeView(v);
	    				container.remove(k);
	    				newFolder.mInfo.add((ApplicationInfo)info);
	    				LauncherModel.addOrMoveItemInDatabase(mLauncher, info, newFolder.mInfo.id, 0, 0, 0);
	    				break;
	    			}
	    		}
	    		if(container.size()==0) {
    	...
    	bindItems.add(folderInfo);
    	Launcher.getModel().addDesktopItem(folderInfo);
        Launcher.getModel().addFolder(folderInfo);
        addInScreen(newFolder, folderInfo.screen, folderInfo.cellX, folderInfo.cellY, 1, 1, false);
        newFolder.refresh();
    	...
    	System.out.println("t.cellX  --- > " + target_info.cellX+", cellY:"+target_info.cellY+", drag cellx:"+mDragInfo.cellX+", drag celly:"+mDragInfo.cellY);
...
//         if(themeFont!=null)((TextView)newFolder).setTypeface(themeFont);
    ...
    public boolean acceptDrop(DragSource source, int x, int y,
        ...
        boolean flag=mVacantCache.findCellForSpan(mTempEstimate, spanX, spanY, false);
//        System.out.println("acceptDrop "+mTempEstimate[0]+", "+mTempEstimate[1]);
        return !flag;
    }
    ...
    private boolean isMergeFolder=false;
    public void onDropCompleted(View target, boolean success) {
        // This is a bit expensive but safe
    	
    	if(!isMergeFolder) return;   // 抽屉内暂不支持文件夹合并
...
//                     if(themeFont!=null)((TextView)newFolder).setTypeface(themeFont);
        ...
        if(mDragInfo!=null && target!=this) {
	        int[] num=new int[3];
			final int maxColumn = mDesktopColumns - 1;
			final int maxRow = mDesktopRows;
	        if(mDragInfo.cellX==maxColumn) {
	        	num[1]=0;
				if(mDragInfo.cellY==maxRow) {
					num[0]=mDragInfo.screen+1;
					num[2]=0;
				}else {
					num[0]=mDragInfo.screen;
					num[2]=mDragInfo.cellY+1;
				}
			}else {
				num[0]=mDragInfo.screen;
				num[1]=mDragInfo.cellX+1;
				num[2]=mDragInfo.cellY;
			}
	        moveItem(num);
	        requestLayout();
        }
    ...
    /*private void moveItem(int[] num) {
    	final int maxColumn = mDesktopColumns - 1;
    	final int maxRow = mDesktopRows - 1;
    	for(int j=num[0];j<getChildCount();j++) {
    		CellLayout cellLayout = (CellLayout) getChildAt(j);
	    	int count=cellLayout.getChildCount();
	    	for(int i=0;i<count;i++) {
	    		View v=cellLayout.getChildAt(i);
	    		ItemInfo info=(ItemInfo)v.getTag();
	    		CellLayout.LayoutParams mParams=(CellLayout.LayoutParams)v.getLayoutParams();
	    		if((mParams.cellX>num[1] && mParams.cellY==num[2]) || mParams.cellX)
	    	}
    	}
    }*/
    ...
    private void moveItem(int[] num) {
    	...
    	for(int j=num[0];j<getChildCount();j++) {
	    	...
	    	for(int i=0;i<count;i++) {
	    		...
	    		ItemInfo info=(ItemInfo)v.getTag();
	    		CellLayout.LayoutParams mParams=(CellLayout.LayoutParams)v.getLayoutParams();
				if (mParams.cellX == num[1] && mParams.cellY == num[2]) {
					if(mParams.cellX==maxColumn) {
			        	num[1]=0;
						if(mParams.cellY==maxRow) {
							num[0]=+1;
							num[2]=0;
						}else {
							num[0]=j;
					...
					/*if(mParams.cellX==maxColumn) {
						...
						num[0]=mParams.cellX+1;
						num[1]=mParams.cellY;
					}*/
					if(mParams.cellX==0) {
						mParams.cellX=maxColumn;
						if(mParams.cellY==0) {
							cellLayout.removeView(v);
							if(cellLayout.getChildCount()==0) {
								removeView(cellLayout);
							}
							mParams.cellY=maxRow;
							info.screen--;
							CellLayout preCellLayout = (CellLayout)getChildAt(info.screen);
							preCellLayout.addView(v, mParams);
						}else {
							mParams.cellY--;
						}
					}else {
					...
					info.cellX=mParams.cellX;
					info.cellY=mParams.cellY;
//					System.out.println(info+", "+mParams.cellX+", "+mParams.cellY+", "+info.container);
//					LauncherModel.addOrMoveItemInDatabase(mLauncher, info, info.container, j, mParams.cellX, mParams.cellY);
					moveItem(num);
					flag=true;
	    			break;
	    		}
    ...
    void indicatorLevels(int mCurrent){
    	/*if(mPreviousIndicator!=null && mNextIndicator!=null) {
...
//        	mLauncher.showPreviews(mLauncher.getDrawerHandle(), 0, getChildCount()); bob
        	invalidate();
            return true;
        }
        return false;
    ...
    public void removeScreen(int screen){
    	...
    	if(mDesktopIndicator!=null)mDesktopIndicator.setItems(getChildCount());
    	indicatorLevels(mCurrentScreen);
        AlmostNexusSettingsHelper.setDesktopScreens(mLauncher, getChildCount(), mLauncher.getMode());
    }
    public CellLayout addScreen(int position){
        LayoutInflater layoutInflter=LayoutInflater.from(mLauncher);
	...
	public void setFolderDragInfo(ApplicationInfo folder_drag_info) {
		this.folderDragInfo = folder_drag_info;
	}
