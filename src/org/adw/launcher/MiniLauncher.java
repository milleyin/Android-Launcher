...
public class MiniLauncher extends ViewGroup implements View.OnLongClickListener, DropTarget,
    ...
    public boolean acceptDrop(DragSource source, int x, int y, int xOffset, int yOffset,
		...
		if ((count == mNumCells && source != this)
				|| ((ItemInfo) dragInfo).itemType == LauncherSettings.Favorites.ITEM_TYPE_USER_FOLDER
				|| ((ItemInfo) dragInfo).itemType == LauncherSettings.Favorites.ITEM_TYPE_APPWIDGET
				|| ((ItemInfo) dragInfo).itemType == LauncherSettings.Favorites.ITEM_TYPE_WIDGET_SEARCH
				|| ((ItemInfo) dragInfo).itemType == LauncherSettings.Favorites.ITEM_TYPE_WIDGET_REMIND_CALENDAR
				|| ((ItemInfo) dragInfo).itemType == LauncherSettings.Favorites.ITEM_TYPE_WIDGET_CLOUD_APP
				|| ((ItemInfo) dragInfo).itemType == LauncherSettings.Favorites.ITEM_TYPE_WIDGET_MY_APP
				|| ((ItemInfo) dragInfo).itemType == LauncherSettings.Favorites.ITEM_TYPE_WIDGET_TASK_MANAGER) {
    		return false;
    	}
		return true;
    }
    ...
    private int FindItemDropPos(int x, int y) {
    	...
    	for(int i=0;i<mNumCells;i++) {
            ...
            Rect r=new Rect(childLeft, childTop, childRight, childBottom);
            if(r.contains(x, y)) {
            	return i;
            }
    	}
    ...
    public void onDrop(DragSource source, int x, int y, int xOffset, int yOffset, Object dragInfo) {
    	if(mDragInfo!=null && mDragInfo.getTag()==dragInfo) {
//    		detachViewFromParent(mDragInfo);
        ...
        if(accept){
	        ...
	        addItemInDockBar(info, source==this, cellX);
...
//    	mBackground.resetTransition();
    ...
    public void addItemInDockBar(ItemInfo info){
        ...
        case LauncherSettings.Favorites.ITEM_TYPE_SHORTCUT:
            ...
            view = mLauncher.createShortcut((ApplicationInfo) info);
...
//        moveToNext(info);
    ...
    public void addItemInDockBar(ItemInfo info, boolean isDragSource, int cellX){
    	View view=null;
        switch (info.itemType) {
        case LauncherSettings.Favorites.ITEM_TYPE_APPLICATION:
        case LauncherSettings.Favorites.ITEM_TYPE_SHORTCUT:
...
//                    (ApplicationInfo) info);
        ...
        System.out.println("isDragSource:"+isDragSource);
        
        moveToNext(info, isDragSource, cellX);
    ...
    /**
     * 如果在 MiniLauncher 的基础上拖拽，则要把拖拽的目标和覆盖的目标调换位置，如果是从其他容器拖拽过来的目标，如果拖拽的目标位置下面有目标，则需要依次往后挪
     * @param info
     * @param isDragSource
     * @param cellX
     * @return
     */
    private int moveToNext(ItemInfo info, boolean isDragSource, int cellX) {
    	int count=getChildCount();
        for(int i=0;i<count;i++) {
        	View child = getChildAt(i);
        	ItemInfo item=(ItemInfo) child.getTag();
        	if(item!=info && item.cellX==info.cellX && item.cellY==info.cellY) {
        		if(isDragSource) {
        			item.cellX=cellX;
        			LauncherModel.moveItemInDatabase(mLauncher, item,
                            LauncherSettings.Favorites.CONTAINER_DOCKBAR, -1, item.cellX, -1);
        			return -1;
        		}
//    			if(mOrientation==HORIZONTAL){
    				if(item.cellX<mNumCells-1) {
    					item.cellX+=1;
    				}else {
    					item.cellX=0;
    				}
    				LauncherModel.moveItemInDatabase(mLauncher, item,
                            LauncherSettings.Favorites.CONTAINER_DOCKBAR, -1, item.cellX, -1);
    				return moveToNext(item, isDragSource, cellX);
//    			}else {
//    				if(item.cellY<)
//    			}
//        		if(item.cellX)
...
//        mDragger.startDrag(v, this, item, DragController.DRAG_ACTION_COPY);
		...
		System.out.println("onlong click:"+item.cellX);
		
        mDragger.startDrag(v, this, item, DragController.DRAG_ACTION_MOVE);
//        detachViewFromParent(v);
//        removeView(v);
        
        mLauncher.mWorkspace.setFolderDragInfo((ApplicationInfo)item);
	...
	protected void onLayout(boolean changed, int l, int t, int r, int b) {
		...
		int marginLeft=((getMeasuredWidth())/2)-(((count*mCellWidth)/2));
		int marginTop=((getMeasuredHeight())/2)-(((count*mCellHeight)/2));
		System.out.println("onLayout:"+count+", marginLeft:"+marginLeft+", marginTop:"+marginTop);
...
//		}
        for (int i = 0; i < count; i++) {
            ...
            if (child.getVisibility() != GONE) {
                ...
                int childTop = (mOrientation==VERTICAL)?marginTop+(item.cellX*mCellHeight):6;//距离下面的距离 
                int childRight = childLeft+mCellWidth;
                int childBottom = childTop+mCellHeight;
		...
		for(int i=0;i<count;i++){
			...
			if (itemInfo.itemType==LauncherSettings.Favorites.ITEM_TYPE_APPLICATION){
	            ...
	            if (icon != null && icon != info.icon) {
	                ...
	                info.filtered = true;
//	                ((BubbleTextView)cell).setImageDrawable(Utilities.drawReflection(info.icon, mLauncher));
	                ((BubbleTextView)cell).setCompoundDrawablesWithIntrinsicBounds(null, info.icon, null,
	        				null);
	            }
			}
        }
	...
	public void onDropCompleted(View target, boolean success) {
		...
		if(target!= this)
		{
			...
			requestLayout();
			mLauncher.mWorkspace.onDropCompleted(target, success);
		}
	}
